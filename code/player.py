...
class Player(pygame.sprite.Sprite):
    def __init__(self, position, groups, collision_sprites, semi_collision_sprites, snail_sprites, frames, data, shoot_arrow):
        '''An instance of the player for the level stages'''
        # general setup
        ...
        # abilities
        self.abilities = {'input': True, 'double_jump': False, 'walljump': False, 'illuminate': True}
        ...
        self.image: pygame.Surface = self.frames[self.state][self.frame_index]
        ...
        self.timers = {
            ...
            'attack_lock': Timer(100),
            'hit': Timer(1200)
        }
        self.timers['spawn'].start()
    
    def take_damage(self) -> None:
        '''Take a hit from enemy or trap damage and lose one health'''
        if not self.timers['hit'].active:
            self.data.health -= 1
            self.timers['hit'].start()
            if self.data.health == 0 and not self.state == 'death':
                self.state = 'death'
                self.frame_index = 0
    ...
    def move(self, dt: float) -> None:
        '''The movement method that applies movement and gravity'''
        if not self.timers['spawn'].active and not self.state == 'death':
            # horizontal movement
    ...
    def platform_move(self, dt: float) -> None:
        '''Move together with platform'''
        if self.platform:
            self.hitbox_rect.topleft += self.platform.direction * self.platform.speed * dt
    
    def check_contact(self) -> None:
        '''Check contact with surfaces for collision logic'''
        floor_rect = pygame.Rect(self.hitbox_rect.bottomleft, (self.hitbox_rect.width, 1))  # rect UNDER player
        right_rect = pygame.Rect((self.hitbox_rect.topright + vector(0, self.hitbox_rect.height / 4)), (1, self.hitbox_rect.height / 2))    # rect on RIGHT side of player
        left_rect = pygame.Rect((self.hitbox_rect.topleft + vector(-1, self.hitbox_rect.height / 4)), (1, self.hitbox_rect.height / 2))    # rect on LEFT side of player
    ...
    def collision(self, axis: str) -> None:
        '''Full collision with surfaces per axis'''
    ...
    def semi_collision(self) -> None:
        '''Semi-collision with surfaces like platforms and snails'''
        if not self.timers['platform_skip'].active:
            groups = self.semi_collision_sprites.sprites() + self.snail_sprites.sprites()
    ...
    def animate(self, dt: float) -> None:
        '''Apply and update animation frames'''
        ...
        if self.state == 'ranged' and int(self.frame_index) == 1 and not self.has_fired:
            self.shoot_arrow(self.hitbox_rect.center, 1 if self.facing_right else -1)
            self.has_fired = True
        
        if self.state in ['melee', 'ranged'] and int(self.frame_index) > len(self.frames[self.state]) - 1:
            self.state = 'idle'
            self.has_fired = False
    ...
    def flicker(self) -> None:
        ...
        if self.timers['hit'].active and not self.state == 'death' and sin(pygame.time.get_ticks() * 50) >= 0:
            change_colours(copy, None, True)
            self.image = copy
